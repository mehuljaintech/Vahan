...
class DataCleaner:
    ...
    def clean_text_columns(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in text_columns:
            try:
                ...
                # Optionally: lowercase other text columns for consistency
                else:
    ...
    def categorize_vehicle(self, row: pd.Series) -> str:
        ...
        """
        try:
            # --- Step 1: Check vehicle class columns ---
            ...
            if vehicle_class_cols:
                ...
                for category, keywords in Config.VEHICLE_CATEGORIES.items():
                    if any(keyword.upper() in vehicle_class for keyword in keywords):
                        self.logger.debug(f"✅ Vehicle categorized by class column '{vehicle_class_cols[0]}': {category}")
                        return category

            # --- Step 2: Fallback to numeric column analysis ---
            numeric_values = {}
            for col in Config.NUMERIC_COLUMNS:
                try:
            ...
            if numeric_values:
                ...
                # Determine category with highest registration count
                max_category = max([
                    (two_w_total, '2W'),
                    (three_w_total, '3W'),
                    (four_w_total, '4W+')
                ...
                category_result = max_category[1] if max_category[0] > 0 else 'Unknown'
                self.logger.debug(f"✅ Vehicle categorized by numeric values: {category_result}")
                return category_result

            # --- Step 3: Default if nothing matches ---
    ...
    def remove_duplicates(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Remove duplicate rows from the dataset, both exact duplicates
        and duplicates based on key columns (State, Year, Vehicle Class).
        ...
        self.logger.debug(f"Exact duplicates removed. Remaining rows: {len(data_copy)}")

        # --- Step 2: Remove duplicates based on key columns if available ---
        ...
        if key_columns:
            ...
            self.logger.debug(f"Duplicates removed based on key columns {key_columns}. Remaining rows: {len(data_copy)}")

        removed_count = initial_count - len(data_copy)
        if removed_count > 0:
            self.logger.info(f"🧹 Removed {removed_count} duplicate rows in total")
        else:
    ...
    def handle_missing_values(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Handle missing values in the dataset for both numeric and text columns.
        ...
        # --- Step 1: Fill numeric columns with 0 ---
        ...
        # --- Step 2: Fill text columns with defaults ---
    ...
    def clean_all(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Perform comprehensive data cleaning: validation, numeric/text cleaning, 
